...
class HistoryPurchaseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            $response = Http::withHeaders($headers)->get($this->base_url . '/api/purchase/history');
    ...
    public function show(Request $request, string $id = null)
    {
        ...
        $headers = $this->set_headers(['Authorization' => $frontendToken]);
        try {
            $response = Http::asForm()->withHeaders($headers)->get($this->base_url . '/api/purchase/history_detail?trx_id=' . $id);
            return $this->handle_response($response);
        } catch (Exception $e) {
            return $this->handle_error($e->getMessage());
        }
